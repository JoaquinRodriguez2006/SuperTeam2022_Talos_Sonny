...
class Robot:
    ...
    def setMaxVelocity(self, vel: float) -> None:
        if not self.inSimulation or self.wb_node == None:
          return
...
def resetControllerFile(num, manual=False) -> None:
    '''Remove the controller'''
    path = os.path.dirname(os.path.abspath(__file__))
    if path[-4:] == "game":
      path = os.path.join(path, "controllers/robot"+str(num)+"Controller")
    else:
      path = os.path.join(path, "../robot"+str(num)+"Controller")

    if configData[0] and not manual:
      files = glob.glob(os.path.join(path, "*"))
      if len(files) > 0:
        supervisor.wwiSendText("loaded"+str(num)+"")
      return
        ...
        if os.path.isfile(robot_proto):
          if configData[0] and not manual:
            if not filecmp.cmp(default_robot_proto, robot_proto):
              supervisor.wwiSendText("loaded2")
            return
          shutil.copyfile(default_robot_proto, robot_proto)
        else:
          shutil.copyfile(default_robot_proto, robot_proto)          
          supervisor.worldReload()
        supervisor.wwiSendText("unloaded2")
    except:
...
def getSwamps(swamps, numberOfSwamps):
    ...
    for i in range(numberOfSwamps):
        ...
        swampPenalty = supervisor.getFromDef("swamp" + str(i) + "pen").getField("penalty").getSFInt32()
...
def relocate(robot, robotObj):
    ...
    robot0Obj.history.enqueue("Lack of Progress")
...
def create_log_str():
    ...
    # Get robot events
    r0_str = robots[0].get_log_str()

    log_str = f"""MAX_GAME_DURATION: {str(maxTimeMinute)}:00
ROBOT_0_SCORE: {str(robots[0].getScore())}

ROBOT_0: {str(robots[0].name)}
{r0_str}"""
...
def add_map_multiplier():  
    score_change = robots[0].getScore() * robots[0].map_score_percent
    robots[0].increaseScore("Map Bonus", score_change)    
...
if __name__ == '__main__':
    ...
    while simulationRunning or last == True:
        ...
        if last == True:
            ...
            finished = True
            if configData[2]:
                # supervisor.setLabel(0, "Score: " + str(round(robots[0].getScore(),2)), 0.15, 0.3,0.3, 0xe74c3c, 0)
        ...
        if first and currentlyRunning:
            ...
            for rb_num in range(len(robots)):
              # Get the robot nodes by their DEF names
              robot0 = supervisor.getFromDef("ROBOT"+str(rb_num))
              # Add robot into world
              add_robot(rb_num)
              # Init robot as object to hold their info
              robots[rb_num] = Robot(robot0)
              # Set robots starting position in world
              set_robot_start_pos(rb_num, robots[rb_num])
              robots[rb_num].inSimulation = True
              robots[rb_num].setMaxVelocity(DEFAULT_MAX_VELOCITY)
              robotInitialized = True

              # Reset physics
              robots[rb_num].wb_node.resetPhysics()
              ...
              #    setViewPoint(robots[rb_num], viewpoint_node, nowSide)
              ...
              #robots[0].increaseScore("Debug", 100)
        ...
        for rb_num in range(len(robots)):
          if robots[rb_num].inSimulation and end_game == False:
              
            
              # if configData[3] and viewpoint_node:
              #   nearVictims = [h for h in humans if h.checkPosition(robots[0].position, 0.20) and h.onSameSide(robots[0].position)]
              #   if len(nearVictims) > 0:
              #     if(len(nearVictims) > 1):
              #       nearVictims.sort(key=lambda v: v.getDistance(robots[0].position))
              #     side = nearVictims[0].getSide()
              #     if side != nowSide:
              #       setViewPoint(robots[0], viewpoint_node, side)
              #       nowSide = side
              
              # Test if the robots are in checkpoints
              checkpoint = [c for c in checkpoints if c.checkPosition(robots[rb_num].position)]
              if len(checkpoint):
                # robots[rb_num].lastVisitedCheckPointPosition = checkpoint[0].center
                alreadyVisited = False

                # Dont update if checkpoint is already visited
                if not any([c == checkpoint[0].center for c in robots[rb_num].visitedCheckpoints]):
                    if superteam:
                      if rb_num == 1 and last != -1 and current_active_robot == 1:
                        end_game = True
                        robots[rb_num].history.enqueue("Finished!")
                      
                    # Update robots[rb_num]'s points and history
                    robots[rb_num].visitedCheckpoints.append(checkpoint[0].center)
                    ...
                    # robots[rb_num].increaseScore("Found checkpoint", 10, roomMult[roomNum])

              # Check if the robots are in swamps
              #inSwamp = any([s.checkPosition(robots[rb_num].position) for s in swamps])
              swamp = [s for s in swamps if s.checkPosition(robots[rb_num].position)]
              inSwamp = len(swamp) > 0
              # Check if robots[rb_num] is in swamp
              if robots[rb_num].inSwamp != inSwamp:
                  robots[rb_num].inSwamp = inSwamp
                  if robots[rb_num].inSwamp:
                      # Cap the robots[rb_num]'s velocity to 2
                      robots[rb_num].setMaxVelocity(2)
                      # Reset physics
                      robots[rb_num].wb_node.resetPhysics()
                      # Update history
                      robots[rb_num].history.enqueue("Entered swamp")
                      
                      timeElapsed += int(swamp[0].penalty)
                      if int(swamp[0].penalty) != 0:
                        robots[rb_num].history.enqueue("Time Penalty: -"+str(swamp[0].penalty)+"s")
                  else:
                      ...
                      robots[rb_num].wb_node.resetPhysics()
                      # Update history
                      robots[rb_num].history.enqueue("Exited swamp")
              ...
              if receiver.getQueueLength() > 0:
                  ...
                  try:
                      if rDataLen == 1:
                        ...
                        robots[rb_num].message = [tup[0].decode("utf-8")]
                      # Victim identification bytes data should be of length = 9
                          ...
                          # Store data recieved
                          robots[rb_num].message = [estimated_victim_position, victimType]
                      else:
                          """
                          ...
                          robots[rb_num].map_data = reshaped_data
                  ...
                  if robots[rb_num].message != []:

                      r0_message = robots[rb_num].message
                      robots[rb_num].message = []

                      # If exit message is correct
                      if r0_message[0] == 'E' and rb_num == 0:
                        # Check robots[rb_num] position is on starting tile
                        #if robots[rb_num].startingTile.checkPosition(robots[rb_num].position):
                        #finished = True
                        #supervisor.wwiSendText("ended")
                        # Update score and history
                        robot_quit(robots[current_active_robot], current_active_robot, True)
                        #last = True
                        robots[1].robot_timeStopped = 0
                        robots[1].stopped = False
                        robots[1].stoppedTime = None
                        if current_active_robot == 0:
                          robots[rb_num].history.enqueue("Robot 1 Finished")
                          current_active_robot = 1
                      
                      elif r0_message[0] == 'L':
                        relocate(robots[current_active_robot].wb_node, robots[current_active_robot])
                        robots[current_active_robot].robot_timeStopped = 0
                        robots[current_active_robot].stopped = False
                        robots[current_active_robot].stoppedTime = None
                        # if configData[3] and viewpoint_node:
                        #   setViewPoint(robots[rb_num], viewpoint_node, nowSide)
              ...
              if currentlyRunning and not finished:
                # Relocate robots[rb_num] if stationary for 20 sec
                if robots[rb_num].timeStopped() >= 20 and rb_num == current_active_robot:
                    if not configData[1]:
                      relocate(robots[rb_num].wb_node, robots[rb_num])
                      # if configData[3] and viewpoint_node:
                      #     setViewPoint(robots[rb_num], viewpoint_node, nowSide)
                    robots[rb_num].robot_timeStopped = 0
                    robots[rb_num].stopped = False
                    robots[rb_num].stoppedTime = None

                if robots[rb_num].position[1] < -0.035 and currentlyRunning and not finished:
                    ##if not configData[1]:
                    relocate(robots[rb_num].wb_node, robots[rb_num])
                      # if configData[3] and viewpoint_node:
                      #     setViewPoint(robots[rb_num], viewpoint_node, nowSide)
              ...
              if robotInitialized:
                # Send the update information to the robots[rb_num] window
                nowScore = robots[rb_num].getScore()
                ...
                if (timeElapsed >= maxTime and last != -1):
                  robots[0].history.enqueue("Times up!")
                  end_game = True
                  
              if not end_game:
                if rb_num == 1 and current_active_robot == 0:
                  robots[1].setMaxVelocity(0)
                  if not robots[1].startingTile.checkPosition(robots[1].position):
                    robots[1].position = robots[1].startingTile.center
                if rb_num == 1 and current_active_robot == 1:
                  robots[1].setMaxVelocity(6.28)
        ...
        if message != "":
            ...
            if len(parts) > 0:
                ...
                if parts[0] == "reset":
                    ...
                    if robots[0].startingTile != None:
                        #Show start tile
                        robots[0].startingTile.wb_node.getField("start").setSFBool(True)
                ...
                if parts[0] == 'relocate':
                    ...
                    if len(data) > 1:
                        if int(data[1]) == current_active_robot:
                          relocate(robots[int(data[1])].wb_node, robots[int(data[1])])
                        # if configData[3] and viewpoint_node:
                        #   setViewPoint(robots[0], viewpoint_node, nowSide)
                ...
                if parts[0] == 'quit':
                    ...
                    if len(data) > 1:
                        if int(data[1]) == 0:
                            if gameStarted:
                               # add_map_multiplier()
                                robots[0].history.enqueue("Manual robot 1 quit!")
        ...
        if end_game and not finished and last != -1:
          # wait(supervisor.getTime() - lastTime + 0.5) # FIXES CRASHES ??? Sometimes
          robot_quit(robots[0], 0, True)
