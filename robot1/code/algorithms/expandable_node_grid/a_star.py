from algorithms.expandable_node_grid.traversable import is_traversable, is_traversable_mini_challenge
...
def a_star(grid, start, end):
    ...
    if not is_traversable(grid, end):
...
def a_star_mini_challenge(grid, start, end):
    #assert is_traversable(grid, start)
    if not is_traversable_mini_challenge(grid, start):
        print("WARNING: Start position is not traversable")

    if not is_traversable_mini_challenge(grid, end):
        print("WARNING: End position is not traversable")
    ...
    while len(openList) > 0:
        ...
        for newPosition in ((0, 1), (0, -1), (-1, 0), (1, 0)):  # Adjacent squares
            ...
            nodePosition = (currentNode.position[0] + (newPosition[0] * 2), currentNode.position[1] + (newPosition[1] * 2))
            # Make sure walkable terrain
            if not is_traversable_mini_challenge(grid, nodePosition):
                continue
            # Create new node
            newNode = aStarNode(currentNode, nodePosition)
            # Append
